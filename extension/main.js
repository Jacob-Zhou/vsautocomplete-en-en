	...
	subscriptions.push(
		vscode.languages.registerCompletionItemProvider(DOCUMENT_SELECTOR, {
			provideCompletionItems: (document, position/*, token*/) => {
				...
				let capitalize = (keyword[0].toUpperCase() == keyword[0]);
				let lowerCase = (keyword[0].toLowerCase() == keyword[0]);

				items.forEach(item => {
					let firstChar = item.label.charAt(0);
					item.label = (capitalize ? firstChar.toUpperCase() : lowerCase ? firstChar.toLowerCase() : firstChar) + item.label.slice(1);
				});
